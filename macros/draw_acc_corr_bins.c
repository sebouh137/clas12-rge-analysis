        ...
        int entry_max)
{
    ...
    for (int plti = 0; plti < plt_size; ++plti) {
        ...
        // Write plot(s).
        for (int pi = 0; pi < pn; ++pi) {
            TCanvas *canvas = new TCanvas(Form("%d", pi), Form("%d", pi),
                    1600, 900);
            canvas->cd();
            plt[pi][plti]->Draw();
            canvas->Update();
            ...
            // --- CUSTOM CODE -------------------------------------------------
            for (int i = 0; i < Q2_BINS_SIZE && pi == 3; ++i) {
            ...
            for (int i = 0; i < NU_BINS_SIZE && pi == 4; ++i) {
                TLine *line = new TLine(
                        nu_bins[i], 0,
                        nu_bins[i], 4000);
                line->SetLineColor(kGreen+2);
            ...
            for (int i = 0; i < ZH_BINS_SIZE && pi == 5; ++i) {
                TLine *line = new TLine(
                        zh_bins[i], 0,
                        zh_bins[i], 5800);
                line->SetLineColor(kGreen+2);
            ...
            for (int i = 0; i < PT2_BINS_SIZE && pi == 6; ++i) {
                TLine *line = new TLine(
                        pt2_bins[i], 0,
                        pt2_bins[i], 12500);
                line->SetLineColor(kGreen+2);
            ...
            for (int i = 0; i < PHIPQ_BINS_SIZE && pi == 7; ++i) {
                TLine *line = new TLine(
                        to_rad(phipq_bins[i]), 0,
                        to_rad(phipq_bins[i]), 3300);
                line->SetLineColor(kGreen+2);
                line->SetLineWidth(1);
                line->Draw("same");
            }
